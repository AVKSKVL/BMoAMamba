...
save_module_path = './dataset/xian_urban/model.pkl'
...
# ==============================#
cnn.cuda()
module_path_stage_1 = './dataset/xian_urban/model_stage1.pkl'
cnn_1 = torch.load(module_path_stage_1)
cnn_1.cuda()

optimizer = torch.optim.Adam(cnn.parameters(), lr=LR, weight_decay=weight_dancy)  # optimize all cnn parameters
...
for epoch in range(EPOCH):
    ...
    for step, (HS, SAR, label, _) in enumerate(train_loader):  # gives batch data, normalize x when iterate train_loader
        ...
        gate, _, _ = cnn_1(HS, SAR)
        output = cnn(HS, SAR, gate)

        loss = loss_func0(output, label)  # cross entropy loss
        ...
        if step % 50 == 0:
            ...
            with torch.no_grad():
                gate_test, ms_patch_test, pan_patch_test = cnn_1(HS_test1, SAR_test1)
                test_output = cnn(HS_test1, SAR_test1, gate_test)
...
for step, (HS, SAR, label, gt_xy) in enumerate(test_loader):
    ...
    with torch.no_grad():
        ...
        gate_test, _, _ = cnn_1(HS, SAR)
        test_output = cnn2(HS, SAR, gate_test)
