...
class Classifier(nn.Module):
    def __init__(self, Classes):
        super().__init__()
        self.classifier_1 = nn.Linear(4 * 64 * 64, 512)
        self.classifier_2 = nn.Linear(512, Classes)
...
class MAMBA(nn.Module):
    def __init__(self, d_model, d_state=24, d_conv=3, expand=1, dt_rank='auto', bias=False, device=None, dtype=None, ):
        ...
        self.A_logs1, self.Ds1, self.dt_projs_weight1, self.dt_projs_bias1 = mamba_init.init_dt_A_D(
            6, 6, 64, dt_scale=1.0, dt_init="random", dt_min=0.001, dt_max=0.1,
        ...
        self.patch = nn.Sequential(
            nn.Conv2d(in_channels=4*expand, out_channels=256*expand, kernel_size=8, stride=8, bias=True),
    ...
    def forward(self, x_in):
        ...
        xs1 = xs1.contiguous().view(b, 64*self.expand * 4, p*q)
        xs2 = xs2.contiguous().view(b, 64*self.expand * 4, p*q)
        xs3 = xs3.contiguous().view(b, 64*self.expand * 4, p*q)
        xs4 = xs4.contiguous().view(b, 64*self.expand * 4, p*q)
        ...
        wh_y2 = torch.transpose(out_y2[:, 1].view(b, -1, p, p), dim0=2, dim1=3).contiguous().view(b, -1, p*q)
        invwh_y2 = torch.transpose(inv_y2[:, 1].view(b, -1, p, p), dim0=2, dim1=3).contiguous().view(b, -1, p*q)
        ...
        wh_y3 = torch.transpose(out_y3[:, 1].view(b, -1, p, p), dim0=2, dim1=3).contiguous().view(b, -1, p*q)
        invwh_y3 = torch.transpose(inv_y3[:, 1].view(b, -1, p, p), dim0=2, dim1=3).contiguous().view(b, -1, p*q)
        ...
        wh_y4 = torch.transpose(out_y4[:, 1].view(b, -1, p, p), dim0=2, dim1=3).contiguous().view(b, -1, p*q)
        invwh_y4 = torch.transpose(inv_y4[:, 1].view(b, -1, p, p), dim0=2, dim1=3).contiguous().view(b, -1, p*q)
        y4 = out_y4[:, 0] + inv_y4[:, 0] + wh_y4 + invwh_y4
        y4 = y4.transpose(dim0=1, dim1=2).contiguous()

        y = torch.cat((y1, y2, y3, y4), dim=-1)

        y = self.out_norm(y).view(b, p, p, -1)
        y = y * z
        out = self.out_proj(y)

        return out

class MoE_Mamba(nn.Module):
    def __init__(self):
        super(MoE_Mamba, self).__init__()
        self.num_expert = 4
        self.norm = nn.LayerNorm(256)

        self.mlp = nn.Sequential(nn.Linear(256, 256 * 4), nn.GELU(), nn.Linear(256 * 4, 256))


        self.mamba1 = MAMBA(256)
        self.mamba2 = MAMBA(256)
        self.mamba3 = MAMBA(256)
        self.mamba4 = MAMBA(256)
        self.experts = []
        self.experts.append(self.mamba1)
        self.experts.append(self.mamba2)
        self.experts.append(self.mamba3)
        self.experts.append(self.mamba4)

    def forward(self, x, gate_output):
        gate_probs = F.softmax(gate_output, dim=-1)

        values, indices = torch.topk(gate_probs, k=1, dim=-1)
        gate_score = torch.zeros_like(gate_probs)
        gate_score.scatter_(-1, indices, values)

        gate_score /= gate_score.sum(dim=-1, keepdim=True)

        expert_outputs = torch.stack([expert(self.norm(x)) for expert in self.experts],
                                     dim=-1)
        output = torch.einsum('bijkl,bl->bijk', expert_outputs, gate_score)
...
class Net(nn.Module):
    def __init__(self,channel_ms,channel_pan, Classes):
        ...
        self.scan_161664_ms = nn.Sequential(
            nn.Conv2d(in_channels=4, out_channels=256, kernel_size=8, stride=8, bias=True),
            nn.BatchNorm2d(256),
            Permute(0, 2, 3, 1))
        ...
        self.mamba_ms_1 = MoE_Mamba()
        self.mamba_pan_1 = MoE_Mamba()
        self.FusionBlock2_2 = Block2_2()
        self.classifier = Classifier(Classes=Classes)


    def forward(self, ms_in, pan_in, gate):
        ms = self.upms(ms_in)
        pan = self.R1(pan_in)
        b, c, h, w = ms.shape
        ...
        msf1 = self.mamba_ms_1(ms_patch, gate)
        panf1 = self.mamba_pan_1(pan_patch, gate)
